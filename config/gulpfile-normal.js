...
function normal(){
    ...
    gulp.task('clean:normal', function() {
    ...
    gulp.task('release', function() {
        ...
        isRelease = true;
        return runSequence(
            ['images:normal', 'less:normal', 'scripts:normal'], ['rev:normal']
        );
    });
    ...
    gulp.task('dev', function() {
        return runSequence(
            ['images:normal', 'less:normal', 'scripts:normal'],
            function() {
                var less_watcher = gulp.watch(m_src.css, ['less:normal']);
                less_watcher.on('change', function(event) {
                    console.log('File ' + event.path + ' was ' + event.type + ', running tasks...');
                })
                var js_watcher = gulp.watch(m_src.js, ['scripts:normal']);
    ...
    /*清除*/
    gulp.task('pc_clean:normal', function() {
        return gulp.src(pc_dist.jscss, { read: false })
            .pipe(clean({ force: true }));
    })

    gulp.task('pc_scripts:normal', function() {
    ...
    gulp.task('pc_less:normal', function() {
    ...
    gulp.task('pc_css:normal', function() {
    ...
    gulp.task('pc_images:normal', function() {
    ...
    gulp.task('pc_rev:normal', function() {
    ...
    gulp.task('pc_release', function() {
        isRelease = true;
        return runSequence(
            ['pc_images:normal', 'pc_less:normal', 'pc_css:normal', 'pc_scripts:normal'], ['pc_rev:normal']
        );
    });
    ...
    gulp.task('pc_dev', function() {
        return runSequence(
            ['pc_images:normal', 'pc_less:normal', 'pc_css:normal', 'pc_scripts:normal'],
            function() {
                //watch监听需要监听路径，不能监听具体后缀名文件，所以此处用cssall
                var less_watcher = gulp.watch(pc_src.cssall, ['pc_less:normal']);
                less_watcher.on('change', function(event) {
                    console.log('File ' + event.path + ' was ' + event.type + ', running tasks...');
                })
